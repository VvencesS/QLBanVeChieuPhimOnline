...
namespace ASP_WebForm_QLBanVeChieuPhimOnline.App_Code.database.QuanLyDatVe
{
    public class Ve
    {
        ...
        public static void Ve_Insert(int maPhim, int maPhong, int maKTG, bool status)
        {
            SqlCommand cmd = new SqlCommand("INSERT INTO [dbo].[tb_Ve]([MaPhim],[MaPhong],[MaKTG],Status) VALUES(@maPhim,@maPhong,@maKTG,@status)");
        ...
        {
            SqlCommand cmd = new SqlCommand("SELECT tb_Ve.[MaVe],tb_Phim.MaPhim,tb_Phim.TenPhim,tb_Phim.AnhDaiDien,tb_TheLoai.MaTheLoai,tb_TheLoai.TenTheLoai" +
                ",tb_DinhDang.MaDinhDang,tb_DinhDang.TenDinhDang, ThoiLuong,tb_Phong.MaPhong,tb_Phong.TenPhong,tb_KTG.MaKTG,tb_KTG.NgayChieu, tb_KTG.GioChieu, " +
                "tb_SoGhe.SoGhe,tb_LoaiGhe.MaLoaiGhe,tb_LoaiGhe.TenLoaiGhe, tb_Gia.MaGia,tb_Gia.DonGia, tb_Ve.Status " +
                "FROM[dbo].[tb_Ve] " +
                "INNER JOIN tb_DatGhe ON tb_DatGhe.MaVe = tb_Ve.MaVe " +
                "INNER JOIN tb_SoGhe ON tb_SoGhe.SoGhe = tb_DatGhe.SoGhe " +
                "INNER JOIN tb_Ghe ON tb_Ghe.SoGhe = tb_SoGhe.SoGhe " +
                "INNER JOIN tb_LoaiGhe ON tb_LoaiGhe.MaLoaiGhe = tb_Ghe.MaLoaiGhe " +
                "INNER JOIN tb_BaoGia ON tb_BaoGia.MaLoaiGhe = tb_LoaiGhe.MaLoaiGhe " +
